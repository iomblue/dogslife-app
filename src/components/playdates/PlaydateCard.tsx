...
const PlaydateCard: React.FC<PlaydateCardProps> = ({ profile, onLike, onPass, onShowOwner, isTop }) => {
    return (
        <div className={`bg-white rounded-2xl shadow-xl overflow-hidden absolute w-full border h-full flex flex-col transition-all duration-300 ${isTop ? '' : 'opacity-0 scale-95'}`} style={{ zIndex: isTop ? 10 : 0 }}>
