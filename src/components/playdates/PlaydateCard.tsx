...
const PlaydateCard: React.FC<PlaydateCardProps> = ({ profile, onLike, onPass, onShowOwner, isTop }) => {
    ...
    return (
        <div className={`bg-white rounded-2xl shadow-xl overflow-hidden absolute w-full border h-full flex flex-col transition-all duration-300 ${isTop && !isDismissed ? '' : 'opacity-0 scale-95'}`} style={{ zIndex: isTop ? 10 : 0 }}>
            ...
            {isTop && (
                <div className="flex justify-around p-4 bg-white">
                    <button onClick={handlePass} className="p-4 rounded-full bg-red-100 text-red-500 hover:bg-red-200 transition-colors">
                    ...
                    <button onClick={handleLike} className="p-4 rounded-full bg-green-100 text-green-500 hover:bg-green-200 transition-colors">
